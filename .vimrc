...
func SetComment()
    ...
    call append(line(".")+14, "#define SQ(x) ((x)*(x))")
    ...
    call append(line(".")+18, "#define asc(a,b) if(a>b)swap(a,b)")
    call append(line(".")+19, "#define des(a,b) if(a<b)swap(a,b)")
    call append(line(".")+20, "#define rep(i, n) for(int i=0;i<(n);++i)")
    call append(line(".")+21, "#define ff(i, n) for(int i=1;i<=(n);++i)")
    call append(line(".")+22, "#define ra(e, u) for(int e=head[(u)];e!=-1;e=E[e].next)")
    call append(line(".")+23, "#define rv(u, j) for(int j=0;j<adj[u].size();++j)")
    call append(line(".")+24, "using namespace std;")
    call append(line(".")+25, "")
    call append(line(".")+26, "typedef pair<int, int >PII;")
    call append(line(".")+27, "typedef long long int64;")
    call append(line(".")+28, "const double PI  = acos(-1.0);")
    call append(line(".")+29, "const int INF = 0x3f3f3f3f;")
    call append(line(".")+30, "const double eps = 1e-8;")
    call append(line(".")+31, "")
